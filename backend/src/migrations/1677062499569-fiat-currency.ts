...
export class fiatCurrency1677062499569 implements MigrationInterface {
  name = 'fiatCurrency1677062499569'
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `CREATE TABLE "fiat_currency"
       (
           ...
           "alphabetic_code" character varying NOT NULL,
           "numeric_code"    integer           NOT NULL,
           "symbol"          character varying NOT NULL,
           "decimal"         integer           NOT NULL,
           CONSTRAINT "UQ_fiat_currency_numeric_code" UNIQUE ("numeric_code"),
           CONSTRAINT "PK_9a362f5a4ce8b5fcb56a46415a3" PRIMARY KEY ("id")
       )`
    )
    ...
    for (const currency of currencies) {
      if (isNaN(currency.decimal)) {
        continue
      }
      await queryRunner.query(
        `INSERT INTO fiat_currency (name, alphabetic_code, numeric_code, symbol, decimal)
         VALUES ('${currency.name}', '${currency.alphabeticCode}', ${currency.numericCode}, '${currency.symbol}',
                 ${currency.decimal})`
      )
    }
  }
  ...
  getCurrencies() {
    ...
    const allCurrencies = this.currenciesDataset.filter((c) => !c.WithdrawalDate).filter((c) => !!c.NumericCode)

    //filter out from allCurrencies all duplicates by alphabeticCode
    const uniqueCurrencies = allCurrencies.filter((c, index) => {
      return allCurrencies.findIndex((c2) => c2.AlphabeticCode === c.AlphabeticCode) === index
    })

    for (const currency of uniqueCurrencies) {
      const symbol = this.currencySymbols[currency.AlphabeticCode]
      currenciesResponse.push({
        symbol: symbol ?? null,
        name: currency.Currency,
        alphabeticCode: currency.AlphabeticCode,
        numericCode: currency.NumericCode,
        decimal: Number.parseInt(currency.MinorUnit) ?? 0
      })
    }
